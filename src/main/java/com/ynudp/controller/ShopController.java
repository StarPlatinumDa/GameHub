...
public class ShopController {
     ...
     */
    @GetMapping("/of/type")
    public Result queryShasdpByType(
            @RequestParam("typeId") Integer typeId,
            @RequestParam(value = "current", defaultValue = "1") Integer current,
            @RequestParam(value = "x",required = false) Double x,// 可有可无
            @RequestParam(value = "y",required = false) Double y
    ) {
        log.info("根据类型分页查询商铺信息,{}",typeId);
//        x=null;
//        y=null;

        return shopService.queryShasdpByType(typeId,current,x,y);
    }
